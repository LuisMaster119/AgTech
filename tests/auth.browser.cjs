...
(async () => {
  ...
  try {
    ...
    await page.getByLabel('Correo electrónico').fill('proveedor@demo.test');
    await page.getByLabel('Contraseña', { exact: true }).fill('incorrecta');
    await page.getByRole('button', { name: 'Iniciar sesión', exact: true }).click();
    await page.getByText('Correo o contraseña incorrectos. Usa las credenciales de demostración indicadas.', { exact: true }).waitFor();
