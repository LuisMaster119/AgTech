...
def test_demo_session_lifecycle(client):
    ...
    result = client.post('/auth/demo/session', json={'email': 'proveedor@demo.test', 'password': 'demo123'})
...
def test_login_requires_matching_mock_credentials(client):
    assert client.post('/auth/demo/session').status_code == 422
    for email, password in [('otro@demo.test', 'demo123'), ('proveedor@demo.test', 'incorrecta')]:
        assert client.post('/auth/demo/session', json={'email': email, 'password': password}).status_code == 401
    assert auth._sessions == {}


def test_expired_and_forged_sessions(client):
    token = client.post('/auth/demo/session', json={'email': 'proveedor@demo.test', 'password': 'demo123'}).json()['accessToken']
...
def test_disabled_mode(client, monkeypatch):
    ...
    assert client.post('/auth/demo/session', json={'email': 'proveedor@demo.test', 'password': 'demo123'}).status_code == 503
...
def test_session_cleanup_and_limit(client):
    ...
    client.post('/auth/demo/session', json={'email': 'proveedor@demo.test', 'password': 'demo123'})
    assert 'expired' not in auth._sessions
    auth._sessions.update({str(i): auth.time.time() + 100 for i in range(1000)})
    assert client.post('/auth/demo/session', json={'email': 'proveedor@demo.test', 'password': 'demo123'}).status_code == 429
